...
describe VCloudSdk::Client, :min, :all do
  ...
  describe "#vdc_exists?" do
    ...
    context "vdc with matching name exists" do
      it "returns true" do
        subject.vdc_exists?(VCloudSdk::Test::Response::OVDC).should be_true
      end
    end

    context "vdc with matching name does not exist" do
      it "returns false" do
        subject.vdc_exists?("xxx").should be_false
      end
    end
  end
  ...
  describe "#catalog_exists?" do
    ...
    context "catalog with matching name exists" do
      it "returns true" do
        subject.catalog_exists?(catalog_name).should be_true
      end
    end

    context "catalog with matching name does not exist" do
      it "returns false" do
        subject.catalog_exists?("xxx").should be_false
      end
    end
  end
